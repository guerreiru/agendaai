...
export function OwnerAvailabilitiesPage() {
  ...
  async function onSubmit(values: AvailabilityForm) {
    ...
    try {
      ...
      if (parsedError.statusCode === 409) {
        setPageError(
          "Conflito de horario: o periodo se sobrepoe a outro existente.",
        );
      } else {
  ...
  async function handleDelete(item: Availability) {
    const confirmed = window.confirm("Deseja excluir este periodo?");
    if (!confirmed) {
      return;
    }
  ...
  async function handleLoadSlots() {
    if (!selectedProfessionalId || !selectedServiceId || !previewDate) {
      setPageError(
        "Selecione profissional, servico e data para consultar slots.",
      );
      return;
    }
  ...
  const groupedByWeekday = useMemo(() => {
    ...
    weekdays.forEach(({ value }) => groups.set(value, []));
    ...
    <section className="space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <header>
        ...
        <p className="text-sm text-slate-600">
          Defina periodos semanais por profissional e visualize os slots
          disponiveis por data.
        </p>
      </header>
      ...
      <section className="grid gap-4 rounded-lg border border-slate-200 p-4 lg:grid-cols-3">
        ...
        <div>
          ...
          >
            Servico para pre-visualizacao de slots
          </label>
          <select
            className="w-full rounded-lg border border-gray-300 px-3 py-2"
            id="serviceSelect"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-4 rounded-lg border border-slate-200 p-4">
          <h2 className="text-lg font-semibold text-slate-800">
            {editingAvailability ? "Editar periodo" : "Adicionar periodo"}
          </h2>
        ...
        <section className="space-y-4 rounded-lg border border-slate-200 p-4">
          ...
          <button
            ...
            type="button"
          >
            {isLoadingSlots ? "Consultando..." : "Consultar slots disponiveis"}
          </button>
              ...
              {slots.map((slot) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">
                    {slot.isAvailable === false ? "Indisponivel" : "Disponivel"}
                  </p>
                </li>
              ))}
            </ul>
            ...
            {weekdays.map((weekday) => {
              ...
              return (
                ...
                >
                  ...
                  {dayItems.length === 0 ? (
                    <p className="text-sm text-slate-500">Sem periodos.</p>
                  ) : (
                    dayItems.map((item) => (
                      <div
