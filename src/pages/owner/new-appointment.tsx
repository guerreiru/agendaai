...
import { listCompanyProfessionals } from "../../services/api/professionals";
...
export function OwnerNewAppointmentPage() {
  ...
  useEffect(() => {
    ...
    const loadProfessionals = async () => {
      ...
      try {
        const data = await listCompanyProfessionals(companyId);
        const onlyProfessionals = data.filter(
          (item) => item.role === "PROFESSIONAL",
        );
        setProfessionals(onlyProfessionals);
      } catch (error) {
  ...
  useEffect(() => {
    if (!companyId || !selectedProfessionalId) {
      setServiceOptions([]);
    ...
    void loadServices();
  }, [companyId, selectedProfessionalId, handleApiError]);
  ...
  useEffect(() => {
    ...
    void loadSlots();
  }, [selectedDate, selectedServiceId, selectedProfessionalId, handleApiError]);

  const canGoStep2 = Boolean(selectedClient);
  const canGoStep3 = Boolean(selectedProfessionalId);
  const canGoStep4 = Boolean(selectedServiceId);
  const canGoStep5 = Boolean(selectedDate && selectedSlot);
    ...
    <section className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">
          Novo agendamento (equipe interna)
        </h1>
        <Link
      ...
      {missingCompany && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          Você precisa vincular/cadastrar uma empresa antes de criar
          agendamentos.
        </div>
      )}
        ...
        <div className="mb-5 flex flex-wrap items-center gap-2">
          {[1, 2, 3, 4, 5].map((current) => (
            <div className="flex items-center gap-2" key={current}>
              ...
              {current < 5 && <span className="h-0.5 w-6 bg-slate-200" />}
            </div>
          ))}
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end">
              <button
                ...
                disabled={!canGoStep2 || missingCompany}
                onClick={() => setStep(2)}
                type="button"
              >
                Próximo
            ...
            <div className="grid gap-2">
              {professionals.map((professional) => (
                ...
                >
                  ...
                  <p className="text-sm text-slate-700">{professional.email}</p>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
              ...
              <p>
                ...
                {sanitizeUserInput(selectedProfessional?.displayName ?? "")}
              </p>
              <p>
            ...
            {createStatus === "success" && (
              <div className="rounded-md border border-green-200 bg-green-50 p-4 text-green-800">
                <p className="font-bold">Agendamento criado com sucesso.</p>
