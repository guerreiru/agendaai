...
export function ProfessionalNewAppointmentPage() {
  ...
  }, [searchQuery, selectedClient, navigate, searchRetryTick]);
  ...
  const handleCreateClient = async () => {
    if (!newClientName || !newClientEmail) {
      setCreateClientError("Nome e email são obrigatórios.");
      return;
    }
    ...
    try {
      const client = await quickCreateClient({
        ...
        // Password will be generated by backend and sent via email
              ...
              <>
                ...
                {searchError && (
                  <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                    <p>{searchError}</p>
                    <button
                      className="mt-2 rounded-md border border-red-300 px-2 py-1 text-xs font-semibold"
                      onClick={() => setSearchRetryTick((prev) => prev + 1)}
                      type="button"
                    >
                      Tentar novamente
                    </button>
